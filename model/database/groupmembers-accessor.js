...
const idField = 'GroupmemberID';
const mutableFields = [
  'GroupmemberUserID',
  'GroupmemberGroupID'
];
const fields = [idField, ...mutableFields];
const whereField = 'GroupmemberGroupID';
const userFields = [
  'UserID',
  'UserFirstname',
  'UserLastname',
...
const extendedTable = `((( ${table} LEFT JOIN Users ON GroupmemberUserID=UserID )
...
accessor.list = async (id) => {
  let sql = `SELECT ${extendedFields} FROM ${extendedTable} WHERE ${whereField}=${id} AND UsertypeName="Student" ORDER BY UserLastname,UserFirstname,UserEmail`;
  try {
